...
public class TextBuilder {
    ...
    private final String[] POSITIONS = {"highest", "second", "third", "fourth", "fifth"};
    ...
    public String replaceQuotations() {
        ...
        for(int i = 0; i < bikeEntries.size(); i++){
            String replaced = FORMAT.replaceAll("<position>", POSITIONS[i])
            .replaceAll("<season>", Utilities.toSeason(bikeEntries.get(i).getSeason()))
            .replaceAll("<day>", Utilities.toWeekday(bikeEntries.get(i).getWeekday()))
            .replaceAll("<month>", Utilities.toMonth(bikeEntries.get(i).getMonth()))
            .replaceAll("<total>", String.valueOf(bikeEntries.get(i).getCasual() + bikeEntries.get(i).getRegistered()))
            .replaceAll("<weather>", WEATHER[bikeEntries.get(i).getWeather()])
